...
public class UnitData {
	...
	Map<Integer,_UnitInfo> unitAndUnitInfoMap = new HashMap<Integer,_UnitInfo>();

	//Map<Integer, Integer> 
	
	//Map<String, List<UnitInfo>> unitTypeListMap = new HashMap<>();
	Map<String, Map<Integer, _UnitInfo>> unitTypeListMap = new HashMap<>();
	//Map<String, Integer> numTryUnits = new HashMap<>();
	...
	public _UnitInfo updateUnitInfo(Unit unit)
	{
		...
		{
			firstSeen = true;
			unitAndUnitInfoMap.put(unit.getID(), new _UnitInfo());
		}
		
//		if (unit.getType().equals(UnitType.Zerg_Extractor) && firstSeen) {
//			Util.println("Extractor (" + unit.getID() + ") morphing...");
//		}
		
		_UnitInfo ui = unitAndUnitInfoMap.get(unit.getID());
		ui.setUnit(unit);
		...
		if (firstSeen)
		{
			__Util.println("Unit First Seen : " + unit.getType().toString() + "(" + unit.getID() + ")");
			
			if(!numCreatedUnits.containsKey(unit.getType().toString())){
				numCreatedUnits.put(unit.getType().toString(), 1);
				//unitTypeListMap.put(unit.getType().toString(), new LinkedList<UnitInfo>());
				unitTypeListMap.put(unit.getType().toString(), new HashMap<Integer, _UnitInfo>());
				
				ui.setSsn(1);
			}else{
				int num = numCreatedUnits.get(unit.getType().toString()) + 1;
				numCreatedUnits.put(unit.getType().toString(), num);
				ui.setSsn(num);
			}
			
			unitTypeListMap.get(unit.getType().toString()).put(ui.getUnitID(), ui);
		...
		if (isBadUnitInfo(ui)) {
			...
			unitAndUnitInfoMap.remove(ui.getUnitID());
			unitTypeListMap.get(unit.getType()).remove(unit.getID());
		}
	...
	public void removeUnit(Unit unit)
	{
		...
		try {
			...
			//unitTypeListMap.get(unit.getType().toString()).remove(unitAndUnitInfoMap.remove(unit.getID()));
			if (unit.getType().equals(UnitType.Zerg_Drone) || unit.getType().equals(UnitType.Zerg_Extractor)) {
				__Util.println("  ~~~ The unit " + unit.getType().toString() + "(" + unit.getID() + ") will be removed... from " + unit.getPlayer().getName());
			}
		
		
			unitTypeListMap.get(unit.getType().toString()).remove(unit.getID());
		} catch (Exception e) {
			__Util.println("UnitData.removeUnit(Unit unit) exception >>>>>> Player : " + unit.getPlayer().getName() + " / Type : " + unit.getType().toString() + " / ID : " + unit.getID() + " <<<<<<<");
			throw e;
		}
	}
	...
	public void removeBadUnits()
	{
		...
		while(it.hasNext())
		{
			_UnitInfo ui = unitAndUnitInfoMap.get(it.next());
			if (isBadUnitInfo(ui))
			{
	...
	public final boolean isBadUnitInfo(final _UnitInfo ui)
	{
		if (ui.getUnit() == null)
		{
			return false;
	...
	public final Map<Integer,_UnitInfo> getUnitAndUnitInfoMap() 
	{ 
...
//	public Map<String, List<UnitInfo>> getUnitTypeListMap() {
//		return unitTypeListMap;
//	}
	
	public Map<String, Map<Integer, _UnitInfo>> getUnitTypeListMap() {
		return unitTypeListMap;
	}
