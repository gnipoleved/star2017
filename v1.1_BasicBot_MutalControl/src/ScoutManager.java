...
//					Iterator<_UniUnitInfotInfo> iter = innerMap.values().iterator();
//					while (iter.hasNext()) {
...
//					}
					...
					for (Unit unit : MyBotModule.Broodwar.self().getUnits()) {
						if (unit != null && unit.exists() && unit.getHitPoints() > 0) {
							//System.out.println(unit.getType().toString() + "(" + unit.getID() +") my unit");
							if (unit.getType() == UnitType.Zerg_Overlord) {
								currentScoutUnit = unit;
								System.out.println("overlord picked up ?? : " + unit.getID());
								break;
							}
						}
					}
	...
	public void moveScoutUnit()
	{
		...
		if (enemyBaseLocation == null)
		{
			...
			if (currentScoutTargetBaseLocation == null || currentScoutUnit.getDistance(currentScoutTargetBaseLocation.getPosition()) < 5 * Config.TILE_SIZE) 
			{
		        ...
		        //System.out.println(currentScoutUnit.getID() + " now moving?");
		    }
		}
