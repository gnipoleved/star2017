...
public class StrategyManager {
	...
	void setVariables(){
		...
		enemyPlayer = _InformationManager.Instance().enemyPlayer;
	...
	boolean isTimeToStartElimination(){
		...
		BaseLocation enemyMainBaseLocation = _InformationManager.Instance().getMainBaseLocation(enemyPlayer);
	...
	void commandMyCombatUnitToEliminate(){
		...
		// 적군의 남은 건물 정보
		_UnitInfo remainingBuildingUnitInfo = null;
		double minDistance = 1000000000;
		double tempDistance = 0;		
		for(Map.Entry<Integer, _UnitInfo> unitInfoEntry : _InformationManager.Instance().getUnitAndUnitInfoMap(enemyPlayer).entrySet()) {
	...
	void updateVariables(){
		...
		enemyRace = _InformationManager.Instance().enemyRace;
		...
		numberOfCompletedEnemyCombatUnit = 0;
		for(Map.Entry<Integer, _UnitInfo> unitInfoEntry : _InformationManager.Instance().getUnitAndUnitInfoMap(enemyPlayer).entrySet()) {
			_UnitInfo enemyUnitInfo = unitInfoEntry.getValue(); 
		...
		// 아군 / 적군의 본진, 첫번째 길목, 두번째 길목
		myMainBaseLocation = _InformationManager.Instance().getMainBaseLocation(myPlayer); 
		myFirstExpansionLocation = _InformationManager.Instance().getFirstExpansionLocation(myPlayer); 
		myFirstChokePoint = _InformationManager.Instance().getFirstChokePoint(myPlayer);
		mySecondChokePoint = _InformationManager.Instance().getSecondChokePoint(myPlayer);
		enemyMainBaseLocation = _InformationManager.Instance().getMainBaseLocation(enemyPlayer);
		enemyFirstExpansionLocation = _InformationManager.Instance().getFirstExpansionLocation(enemyPlayer); 
		enemyFirstChokePoint = _InformationManager.Instance().getFirstChokePoint(enemyPlayer);
		enemySecondChokePoint = _InformationManager.Instance().getSecondChokePoint(enemyPlayer);
	...
	public void executeWorkerTraining() {
		...
		if (MyBotModule.Broodwar.self().minerals() >= 50) {
			...
			int workerCount = MyBotModule.Broodwar.self().allUnitCount(_InformationManager.Instance().getWorkerType());
			...
			for (BaseLocation baseLocation : _InformationManager.Instance().getOccupiedBaseLocations(myPlayer)) {
					...
					if (unit.getType().isResourceDepot()) {
						if (unit.isTraining() == false || unit.getLarva().size() > 0) {
							...
							if (BuildManager.Instance().buildQueue
									.getItemCount(_InformationManager.Instance().getWorkerType(), null) == 0 && eggWorkerCount == 0) {
								// std.cout + "worker enqueue" + std.endl;
								BuildManager.Instance().buildQueue.queueAsLowestPriority(
										new MetaType(_InformationManager.Instance().getWorkerType()), false);
							}
						}
					}
				}
			...
			if (currentSupplyShortage > 0) {
				...
				// 저그 종족이 아닌 경우, 건설중인 Protoss_Pylon, Terran_Supply_Depot 를 센다. Nexus, Command Center 등 건물은 세지 않는다
					...
					onBuildingSupplyCount += ConstructionManager.Instance().getConstructionQueueItemCount(
							_InformationManager.Instance().getBasicSupplyProviderUnitType(), null)
							* _InformationManager.Instance().getBasicSupplyProviderUnitType().supplyProvided();
				}
				...
				if (currentSupplyShortage > onBuildingSupplyCount) {
					...
					if (!BuildManager.Instance().buildQueue.isEmpty()) {
						...
						if (currentItem.metaType.isUnit() 
							&& currentItem.metaType.getUnitType() == _InformationManager.Instance().getBasicSupplyProviderUnitType()) 
						{
							isToEnqueue = false;
						}
					}
					if (isToEnqueue) {
						...
						BuildManager.Instance().buildQueue.queueAsHighestPriority(
								new MetaType(_InformationManager.Instance().getBasicSupplyProviderUnitType()), true);
					}
				}
			}
		}
	...
	void executeBuildingConstruction() {
		...
		// 현재 공격 유닛 생산 건물 갯수
		int numberOfMyCombatUnitTrainingBuilding = myPlayer.allUnitCount(_InformationManager.Instance().getBasicCombatBuildingType());
		numberOfMyCombatUnitTrainingBuilding += BuildManager.Instance().buildQueue.getItemCount(_InformationManager.Instance().getBasicCombatBuildingType());
		numberOfMyCombatUnitTrainingBuilding += ConstructionManager.Instance().getConstructionQueueItemCount(_InformationManager.Instance().getBasicCombatBuildingType(), null);
		...
		if (isPossibleToConstructCombatUnitTrainingBuildingType == true
			...
			// 게이트웨이 / 배럭 / 해처리 증설
			if (BuildManager.Instance().buildQueue.getItemCount(_InformationManager.Instance().getBasicCombatBuildingType()) == 0 ) 
			{
				BuildManager.Instance().buildQueue.queueAsHighestPriority(_InformationManager.Instance().getBasicCombatBuildingType(), 
	...
	public void executeCombatUnitTraining() {
		...
		{
			for(Unit unit : myPlayer.getUnits()) {
				if (unit.getType() == _InformationManager.Instance().getBasicCombatBuildingType()) {
					if (unit.isTraining() == false || unit.getLarva().size() > 0) {
