...
import bwta.Region;
import umojan.Common;
import umojan.InformationManager;
		...
		if (enemyBaseLocation == null)
		{
			if (currentScoutUnit == null || currentScoutUnit.exists() == false || currentScoutUnit.getHitPoints() <= 0)
			{
				...
				if (Common.SELF.getRace() == Race.Zerg) {
					...
					Collection<UnitInfo> overlords = InformationManager.Instance().getUnitData(Common.SELF).unitTypeListMap.get(Zerg_Overlord.toString()).values();
