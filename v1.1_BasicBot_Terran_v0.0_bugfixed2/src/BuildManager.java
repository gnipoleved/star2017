...
//									currentItem.seedLocationStrategy);
							...
							if (currentItem.fixed) {
								...
								if (_Common.Broodwar.getUnitsOnTile(currentItem.seedLocation).size() > 0) desiredPosition = TilePosition.None;
								else desiredPosition = currentItem.seedLocation;
								__Util.println(" fixed ? : " + desiredPosition);
								
							} else {
								desiredPosition = getDesiredPosition(t.getUnitType(), currentItem.seedLocation, currentItem.seedLocationStrategy);
								__Util.println(" not fixed ? : " + desiredPosition);
							}
							...
							if (desiredPosition != TilePosition.None) {
								...
								if (currentItem.fixed) ConstructionManager.Instance().addConstructionTask(t.getUnitType(), desiredPosition, true);
								else ConstructionManager.Instance().addConstructionTask(t.getUnitType(), desiredPosition);
					...
					for (int i = 0; i < t.getUnitType().tileWidth(); ++i) {
						for (int j = 0; j < t.getUnitType().tileHeight(); ++j) {
							...
							for (Unit u : MyBotModule.Broodwar.getUnitsOnTile(tilePos.getX(), tilePos.getY())) {
								...
								//		+ u.getPlayer().getName() + ")");
								if (u.getPlayer() != _InformationManager.Instance().selfPlayer) {
									isBlocked = false;
								}
							}
						}
