...
public class __EventLogger implements __IEventLogger {
	...
	public void logOnUnitCreate(Unit unit) {
		if (logging) {
			...
			__Util.println("~~~~~~ Unit Created : " + unit.getType() + "(" + unit.getID() + ") : " + MyBotModule.Broodwar.getFrameCount());
		}
	}
	...
	public void logOnUnitComplete(Unit unit) {
		if (logging) {
			updateMaxUnitId(unit.getID());
			__Util.println("~~~~~~~ Unit Completed : " + unit.getType() + "(" + unit.getID() + ") : " + MyBotModule.Broodwar.getFrameCount());
		}
	}
