...
public class ConstructionPlaceFinder {
	...
	public final TilePosition getBuildLocationWithSeedPositionAndStrategy(UnitType buildingType, TilePosition seedPosition, BuildOrderItem.SeedPositionStrategy seedPositionStrategy)
	{
		...
		// seedPosition 을 입력하지 않은 경우
			...
			case MainBaseLocation:
				desiredPosition = getBuildLocationNear(buildingType, _InformationManager.Instance().getMainBaseLocation(MyBotModule.Broodwar.self()).getTilePosition());
				break;

			case MainBaseBackYard:
				tempBaseLocation = _InformationManager.Instance().getMainBaseLocation(MyBotModule.Broodwar.self());
				tempChokePoint = _InformationManager.Instance().getFirstChokePoint(MyBotModule.Broodwar.self());
			...
			case FirstExpansionLocation:
				tempBaseLocation = _InformationManager.Instance().getFirstExpansionLocation(MyBotModule.Broodwar.self());
			...
			case SecondChokePoint:
				tempChokePoint = _InformationManager.Instance().getSecondChokePoint(MyBotModule.Broodwar.self());
				if (tempChokePoint != null) {
					desiredPosition = getBuildLocationNear(buildingType, tempChokePoint.getCenter().toTilePosition());
				}
				break;
	...
	public final TilePosition getBuildLocationNear(UnitType buildingType, TilePosition desiredPosition)
	{
		...
		{
			desiredPosition = _InformationManager.Instance().getMainBaseLocation(MyBotModule.Broodwar.self()).getTilePosition();
		}
...
//		if (!MyBotModule.Broodwar.canBuildHere(position, b.getType(), b.getConstructionWorker()))
	...
	public final TilePosition getRefineryPositionNear(TilePosition seedPosition)
	{
		...
		{
			seedPosition = _InformationManager.Instance().getMainBaseLocation(MyBotModule.Broodwar.self()).getTilePosition();
		}
	...
	public final boolean isOverlapsWithBaseLocation(TilePosition tile, UnitType type)
	{
		...
		for (BaseLocation base : BWTA.getBaseLocations())
		{
			...
			int bx2 = bx1 + _InformationManager.Instance().getBasicResourceDepotBuildingType().tileWidth();
			int by2 = by1 + _InformationManager.Instance().getBasicResourceDepotBuildingType().tileHeight();
	...
	public void setTilesToAvoid()
	{
		...
		for (BaseLocation base : BWTA.getBaseLocations())
		{
			...
			if (BWTA.isConnected(base.getTilePosition(), _InformationManager.Instance().getMainBaseLocation(MyBotModule.Broodwar.self()).getTilePosition()) == false) continue;
