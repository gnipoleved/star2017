...
public class StrategyManager {
	...
	List<Unit> mutalGroup;
	
	int frameStart = 0;
	
	public void setUpMutalGroup() {
		
		if (MyBotModule.Broodwar.getFrameCount() % 10 != 0) return;
		
		if (mutalGroup != null && mutalGroup.size() >= 2) return;
			...
			if (unit != null && unit.exists() && unit.getHitPoints() > 0) {
				if (unit.getType() == UnitType.Zerg_Mutalisk && unit.isCompleted()) {
					//if (mutalCount < 11) {
						...
						mutalCount++;
					//}
				} else if (unit.getType() == UnitType.Zerg_Larva && unit.isMorphing()==false) {
					larvaCount++;
					if (larva == null) larva = unit;
				}
			}
		}
		
		//System.out.println("mutal count : " + mutalCount + ", mutal group cnt : " + group.size() + " / larvacnt: " + larvaCount);
		
		if (mutalCount == 11 && larvaCount >= 1) {
			group.add(larva);
	...
	static TilePosition CENTER_POS = new TilePosition(64, 64);
	
	public void controlMutalGroup() {
		
		if (MyBotModule.Broodwar.getFrameCount() % 10 != 0) return;
		
		if (mutalGroup == null || mutalGroup.size() < 2) return;
	
		//mutalGroup.get(0).move(CENTER_POS.toPosition());
		
		//MyBotModule.Broodwar.issueCommand(mutalGroup, mutalGroup.get(0).command    getLastCommand());
		
		
		
		Position targetPosition = CENTER_POS.toPosition(); 
		
		for (Unit mutal : mutalGroup) {
			//mutal.move(targetPosition);
			mutal.rightClick(targetPosition);
		}
