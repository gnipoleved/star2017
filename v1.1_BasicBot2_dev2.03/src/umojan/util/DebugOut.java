...
public class DebugOut {
	...
	public static File getNewLogFileWithCurrentDate(Date currDate) {
		File newFile = new File(LOG_FILE_PFX  + DateUtil.TO_STRING(currDate, "yyyyMMdd_HHmmss") + ".log");
		if (newFile.exists()) {
			newFile = new File(LOG_FILE_PFX  + DateUtil.TO_STRING(new Date(System.currentTimeMillis() + 1000), "yyyyMMdd_HHmmss") + ".log");
		}
		return newFile;
	}
	...
	private static void filelog() throws DebugException {
		...
		try {
			FileUtil.writeTextToFile(file, logs.toString());
		} catch (Throwable e) {
	...
	private static String makeMillisFormatted(long currentMillis) {
		return "[" + DateUtil.TO_STRING(new Date(currentMillis), "yyyy-MM-dd HH:mm:ss") + "] ";
	}
