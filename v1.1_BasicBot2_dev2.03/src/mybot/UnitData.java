...
public class UnitData {
	...
	Map<String, Map<Integer, UnitInfo>> unitTypeListMap = new HashMap<>();
...
//		}
		...
		if (firstSeen)
		{
			Util.println("Unit First Seen : " + unit.getType().toString() + "(" + unit.getID() + ")");
			...
			if(!numCreatedUnits.containsKey(unit.getType().toString())){
				...
				unitTypeListMap.put(unit.getType().toString(), new HashMap<Integer, UnitInfo>());
	...
	public void removeUnit(Unit unit)
	{
		...
		try {
			...
			if (unit.getType().equals(UnitType.Zerg_Drone) || unit.getType().equals(UnitType.Zerg_Extractor)) {
				Util.println("  ~~~ The unit " + unit.getType().toString() + "(" + unit.getID() + ") will be removed... from " + unit.getPlayer().getName());
			}
		
		
			unitTypeListMap.get(unit.getType().toString()).remove(unit.getID());
		} catch (Exception e) {
			Util.println("UnitData.removeUnit(Unit unit) exception >>>>>> Player : " + unit.getPlayer().getName() + " / Type : " + unit.getType().toString() + " / ID : " + unit.getID() + " <<<<<<<");
			throw e;
		}
	}
...
//	}
	...
	public Map<String, Map<Integer, UnitInfo>> getUnitTypeListMap() {
		return unitTypeListMap;
	}
