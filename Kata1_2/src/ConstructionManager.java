...
public class ConstructionManager {
	...
	public void addConstructionTask(UnitType type, TilePosition desiredPosition) {
		addConstructionTask(type, desiredPosition, false);
	}
	
	public void addConstructionTask(UnitType type, TilePosition desiredPosition, boolean fixed)
	{
		...
		b.setFixedDesiredPosition(fixed);
	...
	public void assignWorkersToUnassignedBuildings()
	{
	    ...
	    for (ConstructionTask b : constructionQueue)
	    {
			...
			TilePosition testLocation = null;
			if (b.isFixedDesiredPosition()) {
				//desiredPosition = currentItem.seedLocation;
				
				if (_Common.Broodwar.getUnitsOnTile(b.getDesiredPosition()).size() > 0) testLocation = TilePosition.None;
				else testLocation = b.getDesiredPosition();
				__Util.println(" fixed in ConsturctionManager? : " + testLocation);
				
			} else {
				testLocation = ConstructionPlaceFinder.Instance().getBuildLocationNear(b.getType(), b.getDesiredPosition());
				__Util.println(" not fixed in ConsturctionManager? : " + testLocation);
			}
		...
		if (MyBotModule.Broodwar.self().getRace() == Race.Terran) {
			...
			for (ConstructionTask b : constructionQueue)
			{
				...
				if (b.getStatus() == ConstructionTask.ConstructionStatus.UnderConstruction.ordinal()) {
					...
					// ljw
					//if (b.getConstructionWorker() == null || b.getConstructionWorker().exists() == false || b.getConstructionWorker().getHitPoints() <= 0 || b.getConstructionWorker().isAttacking()){
					if (b.getConstructionWorker() == null || b.getConstructionWorker().exists() == false || b.getConstructionWorker().getHitPoints() <= 0 || WorkerManager.Instance().getWorkerData().getJobCode(b.getConstructionWorker()) != 'B'){
	...
	public void checkForDeadlockConstruction()
	{
		...
		for (ConstructionTask b : constructionQueue)
		{
			if (b.getStatus() != ConstructionTask.ConstructionStatus.UnderConstruction.ordinal())
			{
				...
				if (!isDeadlockCase && unitType == _InformationManager.Instance().getRefineryBuildingType())
				{
				...
				if (!isDeadlockCase
					&& !_InformationManager.Instance().getOccupiedRegions(_InformationManager.Instance().selfPlayer).contains(desiredPositionRegion)
					&& _InformationManager.Instance().getOccupiedRegions(_InformationManager.Instance().enemyPlayer).contains(desiredPositionRegion))
				{
					isDeadlockCase = true;
				}
